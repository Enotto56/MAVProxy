...
if wx is not None:
    ...
    class CatchLeaderFrame(wx.Frame):
        ...
        def __init__(self, ui_state: CatchLeaderUI, title: str):
            super().__init__(None, title=title, size=(480, 360))
            ...
            for widget in (self.leader_text, self.follower_text,
                           ...
                           self.status_text, self.warning_text):
                main_sizer.Add(widget, flag=wx.ALL | wx.EXPAND, border=4)

            button_sizer = wx.BoxSizer(wx.HORIZONTAL)
            for button in (btn_catch, btn_hold, btn_resume, btn_clear):
...
class CatchLeader(mp_module.MPModule):
    ...
    def __init__(self, mpstate):
        ...
        self.leader = VehicleState(
            sysid=self.catch_settings.leader_sysid,
            compid=self.catch_settings.leader_compid,
        )
        self.follower = VehicleState(
            sysid=self.catch_settings.follower_sysid,
            compid=self.catch_settings.follower_compid,
        )
    ...
    def _print_usage(self) -> None:
        print("Usage: catchleader <status|set|catch|hold|resume|goto|clear>")

    def _refresh_sysids(self) -> None:
        self.leader.sysid = self.catch_settings.leader_sysid
        self.leader.compid = self.catch_settings.leader_compid
        self.follower.sysid = self.catch_settings.follower_sysid
        self.follower.compid = self.catch_settings.follower_compid
    ...
    def mavlink_packet(self, msg) -> None:
        ...
        if sysid == self.catch_settings.leader_sysid and compid == self.catch_settings.leader_compid:
            state = self.leader
        elif sysid == self.catch_settings.follower_sysid and compid == self.catch_settings.follower_compid:
            state = self.follower
    ...
    def compute_intercept(self, now: float) -> Optional[Tuple[Tuple[float, float, float], float]]:
        if not (self.leader.is_position_fresh(now, self.catch_settings.position_timeout) and
                self.follower.is_position_fresh(now, self.catch_settings.position_timeout)):
            return None
        ...
        bearing_to_leader = mp_util.gps_bearing(self.follower.lat, self.follower.lon,
                                                self.leader.lat, self.leader.lon)
        if self.leader.vx is not None and self.leader.vy is not None:
